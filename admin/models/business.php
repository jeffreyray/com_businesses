 ...
 * @since		1.6
 */
class BusinessesModelBusiness extends JModelAdmin
{
	/**
	...
	protected function canDelete($record)
	{
		if (!empty($record->id)) {
			...
			return $user->authorise('core.delete', 'com_businesses.business.'.(int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id)) {
			return $user->authorise('core.edit.state', 'com_businesses.business.'.(int) $record->id);
		}
	...
	*/
	public function getTable($type = 'Business', $prefix = 'BusinessesTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_businesses.business', 'business', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_businesses.edit.business.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('business.id') == 0) {
				...
				$data->set('catid', JRequest::getInt('catid', $app->getUserState('com_businesses.businesses.filter.category_id')));
			}
		}
